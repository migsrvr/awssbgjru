...
DEFAULT_TEMPLATES = {
    ...
    "approved": {
        ...
        "body": (
            ...
            "Please join the group chats below to stay updated and coordinated with the organization:\n\n"
        ...
        "variables": ["Applicant Name", "Role Title", "Division Name"],
        "version": 1,
    },
    "revision_requested": {
...
def _format_inline_markdown(text: str) -> str:
    """Converts *italic* to <em>italic</em> in email body text."""
...
def build_html_email(
    ...
    if general_chat_link or division_chat_link or general_qr_base64 or division_qr_base64:
        ...
        if general_chat_link or general_qr_base64:
            item = '<div style="margin: 16px 0 24px 0;">'
            if general_chat_link:
                item += f'<p style="margin: 0 0 8px 0; font-size: 15px; color: #2d3748;"><em>General Group Chat</em>: <a href="{general_chat_link}" style="color: #0073bb; text-decoration: underline;" target="_blank">{general_chat_link}</a></p>'
            else:
                item += '<p style="margin: 0 0 8px 0; font-size: 15px; color: #2d3748;"><em>General Group Chat</em></p>'
            if general_qr_base64:
                item += f'<img src="{general_qr_base64}" alt="General Group Chat QR Code" width="180" style="width: 180px; max-width: 100%; height: auto; object-fit: contain; border: 1px solid #e2e8f0; border-radius: 8px; padding: 6px; background: #ffffff; display: block; margin-top: 8px;" />'
            item += '</div>'
        ...
        if division_chat_link or division_qr_base64:
            ...
            if division_chat_link:
                item += f'<p style="margin: 0 0 8px 0; font-size: 15px; color: #2d3748;"><em>{div_label}</em>: <a href="{division_chat_link}" style="color: #0073bb; text-decoration: underline;" target="_blank">{division_chat_link}</a></p>'
            else:
                item += f'<p style="margin: 0 0 8px 0; font-size: 15px; color: #2d3748;"><em>{div_label}</em></p>'
            if division_qr_base64:
                item += f'<img src="{division_qr_base64}" alt="{div_label} QR Code" width="180" style="width: 180px; max-width: 100%; height: auto; object-fit: contain; border: 1px solid #e2e8f0; border-radius: 8px; padding: 6px; background: #ffffff; display: block; margin-top: 8px;" />'
            item += '</div>'
            gc_items.append(item)
    ...
    for p in paragraphs:
        ...
        if p.lower().startswith("congratulations"):
            content_blocks.append(f'<p style="margin: 0 0 16px 0; font-size: 16px; font-weight: bold; color: #1a202c; line-height: 1.5;">{p_formatted}</p>')
        else:
            p_html = p_formatted.replace("\n", "<br />")
            content_blocks.append(f'<p style="margin: 0 0 16px 0; font-size: 15px; color: #2d3748; line-height: 1.6;">{p_html}</p>')
...
<body style="margin: 0; padding: 20px 0; background-color: #f7fafc; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; -webkit-font-smoothing: antialiased;">
